    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex items-center justify-between bg-gray-800 px-6 py-3 border-b border-gray-700">
        ...
        <div className="flex gap-6 text-sm">
          <span className="text-gray-300">
            R<span className="text-white font-bold text-base">{gameState.round}</span>/{gameState.maxRounds}
          </span>
          {playerOrder.map((pid) => {
            const isMe = pid === uid;
            ...
            return (
              <span key={pid} className={isMe ? 'text-blue-300' : 'text-gray-300'}>
                {getPlayerName(pid)}{isMe ? '(自分)' : ''}:{' '}
                <span className="font-bold text-base text-white">{score}pt</span>
                {isCurrent && (
      ...
      <div className="flex-1 p-6">
          ...
          <div className="flex flex-col items-center justify-center gap-6 py-12">
            {turnPanicked ? (
              ...
              <>
                ...
                <div className="text-2xl text-white">+{turnProfit}点 獲得</div>
                {turnProfit <= 5 && (
        ...
        {localPhase === 'game_over' && (
          <div className="flex flex-col items-center justify-center gap-6 py-12">
            <div className="text-3xl font-bold text-amber-400">ゲーム終了</div>
            ...
            {(() => {
              ...
              if (myFinalScore > opScore) return <p className="text-2xl text-amber-400 font-bold">勝利！</p>;
              if (myFinalScore < opScore) return <p className="text-2xl text-blue-400 font-bold">敗北...</p>;
              return <p className="text-2xl text-gray-300 font-bold">引き分け</p>;
            })()}

            <button
              onClick={onBack}
              className="px-8 py-3 bg-gray-700 hover:bg-gray-600 rounded-lg text-white font-bold cursor-pointer transition-all"
            >
              タイトルに戻る
            </button>
          </div>
